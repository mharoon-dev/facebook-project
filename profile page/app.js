import { auth, db, doc, getDoc, onAuthStateChanged } from "../utilities/fireBaseConfig.mjs";
import { getData, updateData, uploadFile } from "../utilities/functions.mjs";
...
let loggedInUserCheck = onAuthStateChanged(auth, async(user) => {
    ...
    if (docSnap.exists()) {
        ...
        userProfileImage.src = userDetails.profileImage ? userDetails.profileImage : "../assets/home/user account button image.png" 
...
let selectedFile = fileInput.addEventListener('change', async function(event) {

    file = event.target.files[0];
    selectedFileName = `${new Date().getTime()}-${file?.name}`;
    console.log(file);
    console.log(selectedFileName);

    if (file.type !== "video/mp4") {
        console.log(file.type);
        let uploadProfileImage = await uploadFile(file , selectedFileName)
            console.log('your file is uploaded!');
            let saveImage = userDetails.profileImage = uploadProfileImage.downloadURL
            console.log(await userDetails);
...
// userProfileImage.src = await downloadURL ? downloadURL : "../assets/home/user account button image.png"
